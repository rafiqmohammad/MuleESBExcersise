package com.mulesoft.mule.transport.sftp;
...
import com.mulesoft.mule.transport.ftp.EEFtpMessageReceiver;


public class MyCustomSftpMessageReceiver extends EEFtpMessageReceiver {
	public final SftpConnector connector;

	public MyCustomSftpMessageReceiver(Connector connector,
	...
	private List<byte[]> extarctFiles(FTPFile[] files) {
		...
		try {
			//SftpClient
			SftpClient client = connector.get (endpoint.getEndpointURI());
			System.out.println("Inside extractFiles method");
	...
	public void processFile(FTPFile[] file) throws Exception {
		...
		client = connector.getSftp(endpoint.getEndpointURI());
